...
import AddSvg from "../../components/assets/addButton.svg";
...
import { getCollaborators, getPositions } from "../../service/requisitions";
...
export default function Collaborator() {
  ...
  const [filterCollabList, setFilterCollabList] =
    useState<CollaboratorType[]>();
  ...
  useEffect(() => {
    ...
    if (cargos) {
      const CargoSelecionado = cargos.filter(
        (elemente) => elemente.id == cargo
      );
  ...
  function showCollaboratorsRows(collaborators: CollaboratorType[]) {
    return collaborators.map((collaborator) => {
      ...
      if (cargos) {
        colabCareer = cargos.filter(
          (cargo) => cargo.id == collaborator.idCargo
        );
      }
      ...
      return (
        <tr key={collaborator.id} data-id={collaborator.id}>
          <td>
            <Typography variant="body-XS">{collaborator.nome}</Typography>
          </td>
          <td>
            <Typography variant="body-XS">{collaborator.cpf}</Typography>
          </td>
          <td>
            <Typography variant="body-XS">{collaborator.cep}</Typography>
          </td>
          <td>
            <Typography variant="body-XS">{collaborator.logradouro}</Typography>
          </td>
          <td>
            <Typography variant="body-XS">{collaborator.numero}</Typography>
          </td>
          <td>
            <Typography variant="body-XS">{collaborator.cidade}</Typography>
          </td>
          <td>
            <Typography variant="body-XS">{collaborator.estado}</Typography>
          </td>
          <td>
            <Typography variant="body-XS">
              {(colabCareer && colabCareer[0]?.nomeDoCargo) || ""}
            </Typography>
          </td>
          <td>
            <Typography variant="body-XS">
              {(colabCareer && colabCareer[0]?.nivel) || ""}
            </Typography>
          </td>
          <td>
            <Typography variant="body-XS">
              {(colabCareer &&
                colabCareer[0]?.salario.toLocaleString("pt-BR", {
                  style: "currency",
                  currency: "BRL",
                })) ||
                ""}
          ...
          <Conditional condition={isAdmin!}>
            <td>
              ...
              >
                <Typography variant="body-XS">Editar</Typography>
              </Button>
            </td>
            <td>
              <Button
              ...
              >
                <Typography variant="body-XS">Remover</Typography>
              </Button>
            </td>
          </Conditional>
        </tr>
  ...
  function openEditCollaborator(collabId: number) {
    if (collaboratorList) {
      const collabToEdit = collaboratorList.find(
        (collaborator) => collaborator.id == collabId
      );
  ...
  async function deleteUser() {
    try {
      const res = await fetch(
        `${supabaseUrl}/rest/v1/Colaborador?id=eq.${idToEdit}`,
        {
          method: "DELETE",
          headers: {
            apikey,
      ...
      if (res.ok) {
        ...
        showNotification(
          ...
          "O colaborador foi removido com sucesso.",
          "success"
        );
      }
    } catch (error) {
  ...
  function getCollabName(collabId: number) {
    if (collaboratorList) {
      const collab = collaboratorList.find(
        (collaborator) => collaborator.id == collabId
      );
  ...
  function filter(
    ...
    if (cargoId) {
      filteredCollabs = filteredCollabs.filter(
        (collaborator) => collaborator.idCargo === cargoId
      );
    }

    if (salario != "0") {
      filteredCollabs = filteredCollabs.filter((collaborator) => {
        const colabCareer = cargos?.find(
          (cargo) => cargo.id == collaborator.idCargo
        );
      ...
      <Container>
        <Modal isVisible={modalConfirmVisible} onClose={closeModalConfirm}>
          ...
          <DivButtons>
            ...
            >
              <Typography variant="body-XS">Cancelar</Typography>
            </Button>
            <Button size="large" variant="main" onClick={deleteUser}>
              <Typography variant="body-XS">Remover</Typography>
            </Button>
          </DivButtons>
        </Modal>
        ...
        <Modal onClose={closeModal} isVisible={modalIsVisible}>
          <InputContainer>
            ...
            <Input
              height="small"
              value={nivel}
              disabled={postionInputsDisable}
              onChange={(e) => setNivel(e.target.value)}
              textLabel={<Typography variant="body-XS">Nivel</Typography>}
            ></Input>
            <Input
              height="small"
              value={salario}
              ...
              textLabel={<Typography variant="body-XS">Salário</Typography>}
            ></Input>
            <DivButtons>
              ...
              {ModalFunction == "add" ? (
                <Button
                  size="large"
                  variant="main"
                  onClick={RegisterNewCollaborator}
                >
                  <Typography variant="body-XS">Adicionar</Typography>
                </Button>
              ) : (
                <Button size="large" variant="main" onClick={EditUser}>
                  <Typography variant="body-XS">Editar</Typography>
                </Button>
              )}
            </DivButtons>
        ...
        <FunctionsBox>
          <FilterBox>
            ...
            <Input
              ...
              textLabel={<Typography variant="body-M">Nome</Typography>}
            />
            <SelectDesgin
              height="default"
              textLabel="Cargo"
              value={inputCargo}
              onChange={(e) => setInputCargo(Number(e.target.value))}
            >
              <option value="0">
                <Typography variant="body-XS">Selecione Um Cargo</Typography>
              </option>
        ...
        <Table>
          ...
          <table>
            <thead>
              <tr>
                <th>
                  <Typography variant="body-S">Nome</Typography>
                </th>
                <th>
                  <Typography variant="body-S">CPF</Typography>
                </th>
                <th>
                  <Typography variant="body-S">CEP</Typography>
                </th>
                <th>
                  <Typography variant="body-S">Logradouro</Typography>
                </th>
                <th>
                  <Typography variant="body-S">Número</Typography>
                </th>
                <th>
                  <Typography variant="body-S">Cidade</Typography>
                </th>
                <th>
                  <Typography variant="body-S">Estado</Typography>
                </th>
                <th>
                  <Typography variant="body-S">Cargo</Typography>
                </th>
                <th>
                  <Typography variant="body-S">Nível</Typography>
                </th>
                <th>
                  <Typography variant="body-S">Salário</Typography>
                </th>
              </tr>
            </thead>
            <tbody>
            ...
            <Empty
              ...
              text={
                <Typography variant="body-M">
                  Não foram encontrados resultados para este filtro.
                </Typography>
              }
            />
          </DivEmpty>
